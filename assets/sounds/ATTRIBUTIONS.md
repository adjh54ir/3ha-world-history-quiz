...
> **2026-09-16 — 앰비언스 141종 삭제.** 카탈로그(`src/const/ConstMindSounds.ts`)가 사라져 아무 화면도
> 이 파일들을 읽지 않았고, `.m4a` 로만 43MB 를 차지하고 있어 전부 지웠다. 지금 앱에 실려 있는 소리는
> `SoundUtils.ts` 가 require 하는 효과음 9종과 배경음 3종뿐이다. 아래 앰비언트 표는 되살릴 때를 위한 기록이다.
